...
def main():
    ...
    comp_df = pd.read_csv("data/customer29.csv", index_col=0)
    ...
    # Complaints by date
    fig = px.histogram(
        comp_df, x="Date sent to company", template="plotly_white", title="Complaint counts by date"
    )
    ...
    # Pre-processing data
    top_comps = (
        comp_df.groupby("Company")["Date sent to company"]
        .count()
