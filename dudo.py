import plotly.graph_objects as go
import dash
import dash_core_components as dcc
import dash_html_components as html
import matplotlib
import plotly.express as px
import pandas as pd
from datetime import date,datetime
import numpy as np
fig4 = go.Figure()
df= pd.read_csv('data/customer29.csv')
#print(df['Date received'][0])
#date_format = '%m-%d-%y'
cc=0
sss=0
mm=0
for i in range(len(df['Date received'])):
    a=[]
    s=df['Date received'][i]
    #print(s[1])
    for j in range(len(s)):
        if s[j]=='-':
            a.append(int(s[:j]))
            if s[j]==s[j+2]:
                a.append(int(s[j+1:j+2]))
            else:
                a.append(int(s[j+1:j+3]))
            break
    #print(a)
    aa=int(s[-4:])
    a.append(aa)

#    s=df['Date received'][i]
    ss=df['Date sent to company'][i]
    b=[]
    for j in range(len(ss)):
        if ss[j]=='-':
            b.append(int(ss[:j]))
            if ss[j]==ss[j+2]:
                b.append(int(ss[j+1:j+2]))
            else:
                b.append(int(ss[j+1:j+3]))
            break
    aaa=int(ss[-4:])
    b.append(aaa)
    #print(a,b)

    d=abs(date(a[2],a[0],a[1])-date(b[2],b[0],b[1]))
    
   # print(d)
    if d.days==0:
        cc+=1
    elif d.days==1:
        sss+=1
    else:
        mm+=1
    
        
# Create figure
colors = ['gold', 'mediumturquoise', 'darkorange', 'lightgreen']

fig4 = go.Figure(data=[go.Pie(labels=['Resolved within 24hrs','resolved within 48hrs','Unresolved'],
                             values=[cc,sss,mm])])
fig4.update_traces(hoverinfo='label+percent', textinfo='value', textfont_size=20,
                  marker=dict(colors=colors, line=dict(color='#000000', width=2)))
#fig.show()

# Disable the autosize on double click because it adds unwanted margins around the image
# More detail: https://plotly.com/python/configuration-options/
#fig.show(config={'doubleClick': 'reset'})

